  ...
  if (__LL_ADC_IS_ENABLED_ALL_COMMON_INSTANCE(ADCxy_COMMON) == 0UL) {
    ...
    if (ADC_CommonInitStruct->Multimode != LL_ADC_MULTI_INDEPENDENT) {
      MODIFY_REG(ADCxy_COMMON->CCR,
                 ADC_CCR_ADCPRE | ADC_CCR_MULTI | ADC_CCR_DMA | ADC_CCR_DDS |
                     ADC_CCR_DELAY,
...
ErrorStatus LL_ADC_DeInit(ADC_TypeDef *ADCx) {
  ...
  if (READ_BIT(ADCx->CR2, ADC_CR2_ADON) == 0UL) {
    ...
    /* Reset register SR */
    CLEAR_BIT(ADCx->SR,
              (LL_ADC_FLAG_STRT | LL_ADC_FLAG_JSTRT | LL_ADC_FLAG_EOCS |
               LL_ADC_FLAG_OVR | LL_ADC_FLAG_JEOS | LL_ADC_FLAG_AWD1));

    /* Reset register CR1 */
    CLEAR_BIT(ADCx->CR1,
              (ADC_CR1_OVRIE | ADC_CR1_RES | ADC_CR1_AWDEN | ADC_CR1_JAWDEN |
               ADC_CR1_DISCNUM | ADC_CR1_JDISCEN | ADC_CR1_DISCEN |
               ADC_CR1_JAUTO | ADC_CR1_AWDSGL | ADC_CR1_SCAN | ADC_CR1_JEOCIE |
               ADC_CR1_AWDIE | ADC_CR1_EOCIE | ADC_CR1_AWDCH));

    /* Reset register CR2 */
    CLEAR_BIT(ADCx->CR2, (ADC_CR2_SWSTART | ADC_CR2_EXTEN | ADC_CR2_EXTSEL |
    ...
    CLEAR_BIT(ADCx->SMPR2, (ADC_SMPR2_SMP9 | ADC_SMPR2_SMP8 | ADC_SMPR2_SMP7 |
    ...
    /* Reset register SQR1 */
    CLEAR_BIT(ADCx->SQR1, (ADC_SQR1_L | ADC_SQR1_SQ16 | ADC_SQR1_SQ15 |
                           ADC_SQR1_SQ14 | ADC_SQR1_SQ13));

    /* Reset register SQR2 */
    CLEAR_BIT(ADCx->SQR2, (ADC_SQR2_SQ12 | ADC_SQR2_SQ11 | ADC_SQR2_SQ10 |
                           ADC_SQR2_SQ9 | ADC_SQR2_SQ8 | ADC_SQR2_SQ7));

    /* Reset register SQR3 */
    CLEAR_BIT(ADCx->SQR3, (ADC_SQR3_SQ6 | ADC_SQR3_SQ5 | ADC_SQR3_SQ4 |
                           ADC_SQR3_SQ3 | ADC_SQR3_SQ2 | ADC_SQR3_SQ1));

    /* Reset register JSQR */
    CLEAR_BIT(ADCx->JSQR, (ADC_JSQR_JL | ADC_JSQR_JSQ4 | ADC_JSQR_JSQ3 |
...
ErrorStatus LL_ADC_Init(ADC_TypeDef *ADCx, LL_ADC_InitTypeDef *ADC_InitStruct) {
  ...
  if (LL_ADC_IsEnabled(ADCx) == 0UL) {
    ...
    MODIFY_REG(ADCx->CR1, ADC_CR1_RES | ADC_CR1_SCAN,
...
ErrorStatus LL_ADC_REG_Init(ADC_TypeDef *ADCx,
  ...
  if (LL_ADC_IsEnabled(ADCx) == 0UL) {
    ...
    MODIFY_REG(ADCx->CR2,
               ADC_CR2_EXTSEL | ADC_CR2_EXTEN | ADC_CR2_CONT | ADC_CR2_DMA |
                   ADC_CR2_DDS,
...
ErrorStatus LL_ADC_INJ_Init(ADC_TypeDef *ADCx,
  ...
  if (LL_ADC_IsEnabled(ADCx) == 0UL) {
    ...
    if (ADC_INJ_InitStruct->SequencerLength != LL_ADC_REG_SEQ_SCAN_DISABLE) {
      MODIFY_REG(
          ADCx->CR1, ADC_CR1_JDISCEN | ADC_CR1_JAUTO,
    ...
    MODIFY_REG(ADCx->CR2, ADC_CR2_JEXTSEL | ADC_CR2_JEXTEN,
