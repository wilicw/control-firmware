  ...
  *           + Peripheral State and errors functions
  ...
  [..]
   (#) Enable and configure the peripheral to be connected to the DMA Stream
       ...
       necessary) please refer to Reference manual for connection between
  peripherals and DMA requests.

   (#) For a given Stream, program the required configuration through the
  following parameters: Transfer Direction, Source and Destination data formats,
       Circular, Normal or peripheral flow control mode, Stream Priority level,
       Source and Destination Increment mode, FIFO mode and its Threshold (if
  needed), Burst mode for Source and/or Destination (if needed) using
  HAL_DMA_Init() function.

   -@-   Prior to HAL_DMA_Init() the clock must be enabled for DMA through the
  following macros:
  ...
  packing/unpacking: it is possible to set different Data Sizes for the
  Peripheral and the Memory (ie. you can set Half-Word data size for the
  peripheral to access its data register and set Word data size for the Memory
  to gain in access time. Each two half words will be packed and written in a
  single access to a Word in the Memory).

     -@-   When FIFO is disabled, it is not allowed to configure different Data
  Sizes for Source and Destination. In this case the Peripheral Data Size will
  be applied to both Source and Destination.
     ...
     *** DMA HAL driver macros list ***
     ...
     [..]
      ...
      (+) __HAL_DMA_GET_IT_SOURCE: Check whether the specified DMA Stream
...
HAL_StatusTypeDef HAL_DMA_Init(DMA_HandleTypeDef *hdma) {
  ...
  /* the Memory burst and peripheral burst are not used when the FIFO is
   * disabled */
  ...
  if (hdma->Init.FIFOMode == DMA_FIFOMODE_ENABLE) {
    ...
    /* Check compatibility between FIFO threshold level and size of the memory
     * burst */
  ...
  /* Initialize StreamBaseAddress and StreamIndex parameters to be used to
     calculate DMA steam Base Address needed by HAL_DMA_IRQHandler() and
 ...
 * @note   The polling mode is kept in this version for legacy. it is
 * recommended to use the IT model instead. This model could be used for debug
 * purpose.
 * @note   The HAL_DMA_PollForTransfer API cannot be used in circular and double
 * buffering mode (automatic circular mode).
 ...
 * @param  pCallback            pointer to private callback function which has
...
static uint32_t DMA_CalcBaseAndBitshift(DMA_HandleTypeDef *hdma) {
  ...
  static const uint8_t flagBitshiftOffset[8U] = {0U, 6U, 16U, 22U,
                                                 0U, 6U, 16U, 22U};
...
/**
 * @brief  Check compatibility between FIFO threshold level and size of the
 * memory burst
 * @param  hdma       pointer to a DMA_HandleTypeDef structure that contains
 *                     the configuration information for the specified DMA
 * Stream.
