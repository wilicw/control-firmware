/**
  ...
  ******************************************************************************
  ...
  ==============================================================================
  [..]
  (#)Initialize the ADC low level resources by implementing the
  HAL_ADC_MspInit():
  ...
  function:
       ...
       (##) In case of using interrupts (e.g. HAL_ADC_Start_IT())
             (+++) Configure the ADC interrupt priority using
  ...
  (#) ADC driver can be used among three modes: polling, interruption,
     ...
     *** Polling mode IO operation ***
     ...
     [..]
       ...
       (+) Wait for end of conversion using HAL_ADC_PollForConversion(), at this
  stage user can specify the value of timeout according to his end application
       (+) To read the ADC converted values, use the HAL_ADC_GetValue()
  function.
     ...
     *** Interrupt mode IO operation ***
     ...
     [..]
       ...
       (+) Use HAL_ADC_IRQHandler() called under ADC_IRQHandler() Interrupt
  subroutine
       (+) At ADC end of conversion HAL_ADC_ConvCpltCallback() function is
  executed and user can add his own code by customization of function pointer
  HAL_ADC_ConvCpltCallback
       (+) In case of ADC Error, HAL_ADC_ErrorCallback() function is executed
  ...
  HAL_ADC_ErrorCallback
     ...
     *** DMA mode IO operation ***
     ==============================
     [..]
       (+) Start the ADC peripheral using HAL_ADC_Start_DMA(), at this stage the
  user specify the length of data to be transferred at each end of conversion
       (+) At The end of data transfer by HAL_ADC_ConvCpltCallback() function is
  executed and user can add his own code by customization of function pointer
  HAL_ADC_ConvCpltCallback
       (+) In case of transfer Error, HAL_ADC_ErrorCallback() function is
  executed and user can add his own code by customization of function pointer
  HAL_ADC_ErrorCallback
     ...
     *** ADC HAL driver macros list ***
     ...
     [..]
      ...
      (+) __HAL_ADC_GET_IT_SOURCE: Check if the specified ADC interrupt source
  is enabled or disabled
     ...
     [..]
       (@) You can refer to the ADC HAL driver header file for more useful
  macros
  ...
  __HAL_RCC_ADC_RELEASE_RESET().
     (++) ADC clock disable using the equivalent macro/functions as
  configuration step.
               (+++) Example:
  ...
  RCC_OSCILLATORTYPE_HSI;
               (+++) RCC_OscInitStructure.HSIState = RCC_HSI_OFF; (if not used
  for system clock)
  ...
  if (HAL_IS_BIT_SET(hadc->Instance->CR2, ADC_CR2_ADON)) {
    ...
    if (HAL_IS_BIT_CLR(tmpADC_Common->CCR, ADC_CCR_MULTI)) {
      ...
      if ((hadc->Instance == ADC1) ||
          ((hadc->Instance == ADC2) &&
           ((ADC->CCR & ADC_CCR_MULTI_Msk) < ADC_CCR_MULTI_0)) ||
          ((hadc->Instance == ADC3) &&
           ((ADC->CCR & ADC_CCR_MULTI_Msk) < ADC_CCR_MULTI_4))) {
#endif /* ADC2 || ADC3 */
      ...
      /* if instance of handle correspond to ADC1 and  no external trigger
       * present enable software conversion of regular channels */
      if ((hadc->Instance == ADC1) &&
...
__weak void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef* hadc) {
  ...
  UNUSED(hadc);
  /* NOTE : This function Should not be modified, when the callback is needed,
            the HAL_ADC_ConvHalfCpltCallback could be implemented in the user
     file
   */
}
...
__weak void HAL_ADC_LevelOutOfWindowCallback(ADC_HandleTypeDef* hadc) {
  ...
  UNUSED(hadc);
  /* NOTE : This function Should not be modified, when the callback is needed,
            the HAL_ADC_LevelOoutOfWindowCallback could be implemented in the
     user file
   */
}
...
HAL_StatusTypeDef HAL_ADC_ConfigChannel(ADC_HandleTypeDef* hadc,
  ...
  if ((hadc->Instance == ADC1) && (sConfig->Channel == ADC_CHANNEL_VBAT)) {
    /* Disable the TEMPSENSOR channel in case of using board with multiplixed
  ...
  if ((hadc->Instance == ADC1) &&
    ...
    /* Disable the VBAT channel in case of using board with multiplixed
     * ADC_CHANNEL_VBAT & ADC_CHANNEL_TEMPSENSOR*/
    if ((uint16_t)ADC_CHANNEL_TEMPSENSOR == (uint16_t)ADC_CHANNEL_VBAT) {
      tmpADC_Common->CCR &= ~ADC_CCR_VBATE;
    }
