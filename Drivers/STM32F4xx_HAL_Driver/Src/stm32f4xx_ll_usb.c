...
HAL_StatusTypeDef USB_SetTurnaroundTime(USB_OTG_GlobalTypeDef *USBx,
  ...
  stretch enough the USB response time to IN tokens, the USB turnaround time, so
  to compensate for the longer AHB read access latency to the Data FIFO */
  ...
  if (ep->is_in == 1U) {
    ...
    if (((USBx_OUTEP(epnum)->DOEPCTL) & USB_OTG_DOEPCTL_USBAEP) == 0U) {
      USBx_OUTEP(epnum)->DOEPCTL |=
          (ep->maxpacket & USB_OTG_DOEPCTL_MPSIZ) | ((uint32_t)ep->type << 18) |
...
HAL_StatusTypeDef USB_ActivateDedicatedEndpoint(
    const USB_OTG_GlobalTypeDef *USBx, const USB_OTG_EPTypeDef *ep) {
  uint32_t USBx_BASE = (uint32_t)USBx;
  uint32_t epnum = (uint32_t)ep->num;
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef USB_DeactivateDedicatedEndpoint(
    const USB_OTG_GlobalTypeDef *USBx, const USB_OTG_EPTypeDef *ep) {
  uint32_t USBx_BASE = (uint32_t)USBx;
  uint32_t epnum = (uint32_t)ep->num;
    ...
    if (ep->xfer_len == 0U) {
      ...
      if (epnum == 0U) {
        ...
        if (ep->type == EP_TYPE_ISOC) {
          ...
          USBx_INEP(epnum)->DIEPTSIZ |=
              (USB_OTG_DIEPTSIZ_MULCNT & (pktcnt << 29));
        }
      }
...
HAL_StatusTypeDef USB_HC_Halt(const USB_OTG_GlobalTypeDef *USBx,
  ...
  /* In buffer DMA, Channel disable must not be programmed for non-split
     periodic channels. At the end of the next uframe/frame (in the worst case),
     the core generates a channel halted and disables the channel automatically.
   */
