/**
  ...
  ******************************************************************************
  ...
  HAL_ADCEx_InjectedGetValue() function.
  ...
  subroutine
       (+) At ADC end of conversion HAL_ADCEx_InjectedConvCpltCallback()
  function is executed and user can add his own code by customization of
  function pointer HAL_ADCEx_InjectedConvCpltCallback
       (+) In case of ADC Error, HAL_ADCEx_InjectedErrorCallback() function is
  ...
  HAL_ADCEx_InjectedErrorCallback
     ...
     *** Multi mode ADCs Regular channels configuration ***
     ======================================================
     [..]
       (+) Select the Multi mode ADC regular channels features (dual or triple
  mode) and configure the DMA mode using HAL_ADCEx_MultiModeConfigChannel()
  functions.
       (+) Start the ADC peripheral using HAL_ADCEx_MultiModeStart_DMA(), at
  this stage the user specify the length of data to be transferred at each end
  of conversion
       (+) Read the ADCs converted values using the
  HAL_ADCEx_MultiModeGetValue() function.
...
/**
 * @brief  Enables ADC DMA request after last transfer (Multi-ADC mode) and
 ...
 * @param  hadc pointer to a ADC_HandleTypeDef structure that contains
 *         the configuration information for the specified ADC.
 * @param  pData   Pointer to buffer in which transferred from ADC peripheral to
...
__weak void HAL_ADCEx_InjectedConvCpltCallback(ADC_HandleTypeDef* hadc) {
  ...
  /* NOTE : This function Should not be modified, when the callback is needed,
            the HAL_ADC_InjectedConvCpltCallback could be implemented in the
     user file
   */
}
...
HAL_StatusTypeDef HAL_ADCEx_InjectedConfigChannel(
  ...
  if (sConfigInjected->InjectedChannel > ADC_CHANNEL_9) {
    ...
    hadc->Instance->SMPR1 |= ADC_SMPR1(sConfigInjected->InjectedSamplingTime,
  ...
  } else /* ADC_Channel include in ADC_Channel_[0..9] */
  {
    ...
    /* Set the new sample time */
    hadc->Instance->SMPR2 |= ADC_SMPR2(sConfigInjected->InjectedSamplingTime,
                                       sConfigInjected->InjectedChannel);
  }
  ...
  hadc->Instance->JSQR &=
      ~ADC_JSQR(ADC_JSQR_JSQ1, sConfigInjected->InjectedRank,
                sConfigInjected->InjectedNbrOfConversion);

  /* Set the SQx bits for the selected rank */
  hadc->Instance->JSQR |=
      ADC_JSQR(sConfigInjected->InjectedChannel, sConfigInjected->InjectedRank,
               sConfigInjected->InjectedNbrOfConversion);
  ...
  /* if ADC1 Channel_16 or Channel_17 is selected enable TSVREFE
   * Channel(Temperature sensor and VREFINT) */
  if ((hadc->Instance == ADC1) &&
