...
/** @defgroup PWR_Regulator_state_in_STOP_mode PWR Regulator state in SLEEP/STOP
 * mode
 * @{
 */
 ...
 * event was received from the WKUP pin or from the RTC alarm (Alarm A or Alarm
 * B), RTC Tamper event, RTC TimeStamp event or RTC Wakeup. An additional wakeup
 * event is detected if the WKUP pin is enabled (by setting the EWUP bit) when
 * the WKUP pin level is already high.
 ...
 * reset when the device wakes up from Standby mode or by a system reset or
 * power reset.
...
#define PWR_EXTI_LINE_PVD                                                    \
  ((uint32_t)EXTI_IMR_MR16) /*!< External interrupt line 16 Connected to the \
                               PVD EXTI Line */
/**
 * @}
 */
...
#define CR_DBP_BB \
  (uint32_t)(PERIPH_BB_BASE + (PWR_CR_OFFSET_BB * 32U) + (DBP_BIT_NUMBER * 4U))

/* Alias word address of PVDE bit */
#define PVDE_BIT_NUMBER PWR_CR_PVDE_Pos
#define CR_PVDE_BB \
  (uint32_t)(PERIPH_BB_BASE + (PWR_CR_OFFSET_BB * 32U) + (PVDE_BIT_NUMBER * 4U))

/* Alias word address of VOS bit */
#define VOS_BIT_NUMBER PWR_CR_VOS_Pos
#define CR_VOS_BB \
  (uint32_t)(PERIPH_BB_BASE + (PWR_CR_OFFSET_BB * 32U) + (VOS_BIT_NUMBER * 4U))
/**
 * @}
 */
...
#define CSR_EWUP_BB \
  (PERIPH_BB_BASE + (PWR_CSR_OFFSET_BB * 32U) + (EWUP_BIT_NUMBER * 4U))
/**
 * @}
 */
...
/** @defgroup PWR_IS_PWR_Definitions PWR Private macros to check input
 * parameters
 * @{
 */
