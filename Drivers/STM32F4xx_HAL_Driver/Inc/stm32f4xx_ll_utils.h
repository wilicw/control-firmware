...
typedef struct {
  ...
  uint32_t
      PLLN; /*!< Multiplication factor for PLL VCO output clock.
                 This parameter must be a number between Min_Data = @ref
               RCC_PLLN_MIN_VALUE and Max_Data = @ref RCC_PLLN_MIN_VALUE

                 This feature can be modified afterwards using unitary function
                 @ref LL_RCC_PLL_ConfigDomain_SYS(). */

  uint32_t
      PLLP; /*!< Division for the main system clock.
...
#define LL_UTILS_PACKAGETYPE_WLCSP168_FBGA169_LQFP100_LQFP64_UFQFPN48   \
  0x00000100U /*!< WLCSP168 or FBGA169 or LQFP100 or LQFP64 or UFQFPN48 \
                 package type  */
