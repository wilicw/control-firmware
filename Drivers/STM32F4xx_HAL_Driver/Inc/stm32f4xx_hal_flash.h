...
typedef struct {
  ...
  __IO uint8_t VoltageForErase; /*Internal variable to provide voltage range
                                   selected by user in IT context*/

  __IO uint32_t Sector; /*Internal variable to define the current sector which
                           is erasing*/
...
#define FLASH_FLAG_OPERR \
  FLASH_SR_SOP /*!< FLASH operation Error flag                */
...
#define __HAL_FLASH_DISABLE_IT(__INTERRUPT__) \
  (FLASH->CR &= ~(uint32_t)(__INTERRUPT__))
 ...
 *           (*) FLASH_FLAG_RDERR is not available for
 * STM32F405xx/407xx/415xx/417xx devices
...
/** @defgroup FLASH_IS_FLASH_Definitions FLASH Private macros to check input
 * parameters
 * @{
 */
