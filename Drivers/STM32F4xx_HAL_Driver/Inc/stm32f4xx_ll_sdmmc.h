 ...
 */
typedef struct {
  uint32_t ClockEdge; /*!< Specifies the clock transition on which the bit
  ...
  uint32_t ClockBypass; /*!< Specifies whether the SDMMC Clock divider bypass is
                           ...
                           SDMMC_LL_Clock_Bypass               */

  uint32_t
      ClockPowerSave; /*!< Specifies whether SDMMC Clock output is enabled or
                           disabled when the bus is idle.
                           This parameter can be a value of @ref
                         SDMMC_LL_Clock_Power_Save           */

  uint32_t
      BusWide; /*!< Specifies the SDMMC bus width.
...
typedef struct {
  uint32_t
      Argument; /*!< Specifies the SDMMC command argument which is sent
                     to a card as part of a command message. If a command
                     contains an argument, it must be loaded into this register
                     before writing the command to the command register. */

  uint32_t CmdIndex; /*!< Specifies the SDMMC command index. It must be Min_Data
                        = 0 and Max_Data = 64 */
  ...
  uint32_t
      WaitForInterrupt; /*!< Specifies whether SDMMC wait for interrupt request
                           is enabled or disabled. This parameter can be a value
                           of @ref SDMMC_LL_Wait_Interrupt_State  */

  uint32_t
      CPSM; /*!< Specifies whether SDMMC Command path state machine (CPSM)
 ...
 */
typedef struct {
  uint32_t DataTimeOut; /*!< Specifies the data timeout period in card bus clock
                           periods.  */

  uint32_t
      DataLength; /*!< Specifies the number of data bytes to be transferred. */

  uint32_t DataBlockSize; /*!< Specifies the data block size for block transfer.
                               This parameter can be a value of @ref
                             SDMMC_LL_Data_Block_Size    */
  ...
  uint32_t TransferMode; /*!< Specifies whether data transfer is in stream or
                            block mode. This parameter can be a value of @ref
                            SDMMC_LL_Transfer_Type      */

  uint32_t
      DPSM; /*!< Specifies whether SDMMC Data path state machine (DPSM)
                 is enabled or disabled.
...
#define SDMMC_CMD_SEND_OP_COND                                             \
  1U /*!< Sends host capacity support information and activates the card's \
        initialization process. */
#define SDMMC_CMD_ALL_SEND_CID                                               \
  2U /*!< Asks any card connected to the host to send the CID numbers on the \
        CMD line.             */
...
#define SDMMC_CMD_SDMMC_SEN_OP_COND                                           \
  5U /*!< Sends host capacity support information (HCS) and asks the accessed \
        card to send its operating condition register (OCR) content in the                            \
        response on the CMD line.              */
#define SDMMC_CMD_HS_SWITCH                                                  \
  6U /*!< Checks switchable function (mode 0) and switch card function (mode \
        1).                   */
#define SDMMC_CMD_SEL_DESEL_CARD                                              \
  7U /*!< Selects the card by its own relative address and gets deselected by \
        any other address    */
#define SDMMC_CMD_HS_SEND_EXT_CSD                                              \
  8U /*!< Sends SD Memory Card interface condition, which includes host supply \
...
#define SDMMC_CMD_SET_BLOCKLEN                                               \
  16U /*!< Sets the block length (in bytes for SDSC) for all following block \
         commands (read, write, lock). Default block length is fixed to 512                   \
         Bytes. Not effective for SDHS and SDXC. */
#define SDMMC_CMD_READ_SINGLE_BLOCK                                       \
  17U /*!< Reads single block of size selected by SET_BLOCKLEN in case of \
         SDSC, and a block of fixed 512 bytes in case of SDHC and SDXC. */
#define SDMMC_CMD_READ_MULT_BLOCK                                     \
  18U /*!< Continuously transfers data blocks from card to host until \
...
#define SDMMC_CMD_WRITE_SINGLE_BLOCK                                       \
  24U /*!< Writes single block of size selected by SET_BLOCKLEN in case of \
         SDSC, and a block of fixed 512 bytes in case of SDHC and SDXC. */
...
#define SDMMC_CMD_SD_ERASE_GRP_START                                       \
  32U /*!< Sets the address of the first write block to be erased. (For SD \
         card only).              */
#define SDMMC_CMD_SD_ERASE_GRP_END                                             \
  33U /*!< Sets the address of the last write block of the continuous range to \
         be erased.           */
#define SDMMC_CMD_ERASE_GRP_START                                           \
  35U /*!< Sets the address of the first write block to be erased. Reserved \
         for each command system set by switch function command (CMD6). */
#define SDMMC_CMD_ERASE_GRP_END                                                \
  36U /*!< Sets the address of the last write block of the continuous range to \
         be erased. Reserved for each command system set by switch function                       \
         command (CMD6).                  */
...
#define SDMMC_CMD_LOCK_UNLOCK                                                \
  42U /*!< Sets/resets the password or lock/unlock the card. The size of the \
         data block is set by the SET_BLOCK_LEN command. */
#define SDMMC_CMD_APP_CMD                                                \
  55U /*!< Indicates to the card that the next command is an application \
         specific command rather than a standard command. */
#define SDMMC_CMD_GEN_CMD                                                    \
  56U /*!< Used either to transfer a data block to the card or to get a data \
...
#define SDMMC_CMD_APP_SD_SET_BUSWIDTH                                          \
  6U /*!< (ACMD6) Defines the data bus width to be used for data transfer. The \
        allowed data bus widths are given in SCR register. */
#define SDMMC_CMD_SD_APP_STATUS \
  13U /*!< (ACMD13) Sends the SD status. */
#define SDMMC_CMD_SD_APP_SEND_NUM_WRITE_BLOCKS                             \
  22U /*!< (ACMD22) Sends the number of the written (without errors) write \
         blocks. Responds with 32bit+CRC data block. */
#define SDMMC_CMD_SD_APP_OP_COND                                               \
  41U /*!< (ACMD41) Sends host capacity support information (HCS) and asks the \
...
#define SDMMC_CMD_SD_APP_SET_CLR_CARD_DETECT                                   \
  42U /*!< (ACMD42) Connect/Disconnect the 50 KOhm pull-up resistor on CD/DAT3 \
         (pin 1) of the card  */
...
#define SDIO_STOPTRANSFERTIMEOUT \
  100000000U /* Timeout for STOP TRANSMISSION command */
...
#define IS_SDIO_BUS_WIDE(WIDE)                                     \
  (((WIDE) == SDIO_BUS_WIDE_1B) || ((WIDE) == SDIO_BUS_WIDE_4B) || \
...
#define IS_SDIO_CPSM(CPSM) \
  (((CPSM) == SDIO_CPSM_DISABLE) || ((CPSM) == SDIO_CPSM_ENABLE))
/**
 * @}
 */
...
#define SDIO_DATABLOCK_SIZE_8B \
  (SDIO_DCTRL_DBLOCKSIZE_0 | SDIO_DCTRL_DBLOCKSIZE_1)
#define SDIO_DATABLOCK_SIZE_16B SDIO_DCTRL_DBLOCKSIZE_2
#define SDIO_DATABLOCK_SIZE_32B \
  (SDIO_DCTRL_DBLOCKSIZE_0 | SDIO_DCTRL_DBLOCKSIZE_2)
#define SDIO_DATABLOCK_SIZE_64B \
  (SDIO_DCTRL_DBLOCKSIZE_1 | SDIO_DCTRL_DBLOCKSIZE_2)
...
#define SDIO_DATABLOCK_SIZE_1024B \
  (SDIO_DCTRL_DBLOCKSIZE_1 | SDIO_DCTRL_DBLOCKSIZE_3)
...
#define IS_SDIO_DPSM(DPSM) \
  (((DPSM) == SDIO_DPSM_DISABLE) || ((DPSM) == SDIO_DPSM_ENABLE))
/**
 * @}
 */
...
#define SDIO_STATIC_CMD_FLAGS                                               \
  ((uint32_t)(SDIO_FLAG_CCRCFAIL | SDIO_FLAG_CTIMEOUT | SDIO_FLAG_CMDREND | \
              SDIO_FLAG_CMDSENT))

#define SDIO_STATIC_DATA_FLAGS                                               \
  ((uint32_t)(SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_TXUNDERR | \
...
#define CLKCR_CLKEN_BB \
  (PERIPH_BB_BASE + (CLKCR_OFFSET * 32U) + (CLKEN_BITNUMBER * 4U))
...
#define CMD_SDIOSUSPEND_BB \
  (PERIPH_BB_BASE + (CMD_OFFSET * 32U) + (SDIOSUSPEND_BITNUMBER * 4U))

/* Alias word address of ENCMDCOMPL bit */
#define ENCMDCOMPL_BITNUMBER 0x0CU
#define CMD_ENCMDCOMPL_BB \
  (PERIPH_BB_BASE + (CMD_OFFSET * 32U) + (ENCMDCOMPL_BITNUMBER * 4U))

/* Alias word address of NIEN bit */
#define NIEN_BITNUMBER 0x0DU
#define CMD_NIEN_BB \
  (PERIPH_BB_BASE + (CMD_OFFSET * 32U) + (NIEN_BITNUMBER * 4U))

/* Alias word address of ATACMD bit */
#define ATACMD_BITNUMBER 0x0EU
#define CMD_ATACMD_BB \
  (PERIPH_BB_BASE + (CMD_OFFSET * 32U) + (ATACMD_BITNUMBER * 4U))
...
#define DCTRL_DMAEN_BB \
  (PERIPH_BB_BASE + (DCTRL_OFFSET * 32U) + (DMAEN_BITNUMBER * 4U))

/* Alias word address of RWSTART bit */
#define RWSTART_BITNUMBER 0x08U
#define DCTRL_RWSTART_BB \
  (PERIPH_BB_BASE + (DCTRL_OFFSET * 32U) + (RWSTART_BITNUMBER * 4U))

/* Alias word address of RWSTOP bit */
#define RWSTOP_BITNUMBER 0x09U
#define DCTRL_RWSTOP_BB \
  (PERIPH_BB_BASE + (DCTRL_OFFSET * 32U) + (RWSTOP_BITNUMBER * 4U))

/* Alias word address of RWMOD bit */
#define RWMOD_BITNUMBER 0x0AU
#define DCTRL_RWMOD_BB \
  (PERIPH_BB_BASE + (DCTRL_OFFSET * 32U) + (RWMOD_BITNUMBER * 4U))

/* Alias word address of SDIOEN bit */
#define SDIOEN_BITNUMBER 0x0BU
#define DCTRL_SDIOEN_BB \
  (PERIPH_BB_BASE + (DCTRL_OFFSET * 32U) + (SDIOEN_BITNUMBER * 4U))
/**
 * @}
 */
...
#define CLKCR_CLEAR_MASK                                                  \
  ((uint32_t)(SDIO_CLKCR_CLKDIV | SDIO_CLKCR_PWRSAV | SDIO_CLKCR_BYPASS | \
              SDIO_CLKCR_WIDBUS | SDIO_CLKCR_NEGEDGE | SDIO_CLKCR_HWFC_EN))

/* --- DCTRL Register ---*/
/* SDIO DCTRL Clear Mask */
#define DCTRL_CLEAR_MASK                                               \
  ((uint32_t)(SDIO_DCTRL_DTEN | SDIO_DCTRL_DTDIR | SDIO_DCTRL_DTMODE | \
              SDIO_DCTRL_DBLOCKSIZE))

/* --- CMD Register ---*/
/* CMD Register clear mask */
#define CMD_CLEAR_MASK                                                   \
  ((uint32_t)(SDIO_CMD_CMDINDEX | SDIO_CMD_WAITRESP | SDIO_CMD_WAITINT | \
              SDIO_CMD_WAITPEND | SDIO_CMD_CPSMEN | SDIO_CMD_SDIOSUSPEND))

/* SDIO Initialization Frequency (400KHz max) */
#define SDIO_INIT_CLK_DIV \
  ((uint8_t)0x76) /* 48MHz / (SDMMC_INIT_CLK_DIV + 2) < 400KHz */

/* SDIO Data Transfer Frequency (25MHz max) */
#define SDIO_TRANSFER_CLK_DIV \
  ((uint8_t)0x0) /* 48MHz / (SDMMC_TRANSFER_CLK_DIV + 2) < 25MHz */
/**
 * @}
 */
...
#define __SDIO_DISABLE(__INSTANCE__) \
  (*(__IO uint32_t *)CLKCR_CLKEN_BB = DISABLE)
...
#define __SDIO_DMA_ENABLE(__INSTANCE__) \
  (*(__IO uint32_t *)DCTRL_DMAEN_BB = ENABLE)
...
#define __SDIO_DMA_DISABLE(__INSTANCE__) \
  (*(__IO uint32_t *)DCTRL_DMAEN_BB = DISABLE)
 ...
 * interrupt
 *            @arg SDIO_IT_DATAEND:  Data end (data counter, DATACOUNT, is zero)
 * interrupt
 ...
 *            @arg SDIO_FLAG_DATAEND:  Data end (data counter, DATACOUNT, is
 * zero)
 ...
 */
#define __SDIO_GET_FLAG(__INSTANCE__, __FLAG__) \
 ...
 * @retval None
 */
#define __SDIO_CLEAR_FLAG(__INSTANCE__, __FLAG__) \
...
#define __SDIO_GET_IT \
  (__INSTANCE__,      \
   __INTERRUPT__)(((__INSTANCE__)->STA & (__INTERRUPT__)) == (__INTERRUPT__))
 ...
 * @retval None
 */
#define __SDIO_CLEAR_IT(__INSTANCE__, __INTERRUPT__) \
 ...
 * @retval None
 */
#define __SDIO_START_READWAIT_ENABLE(__INSTANCE__) \
  (*(__IO uint32_t *)DCTRL_RWSTART_BB = ENABLE)
 ...
 * @retval None
 */
#define __SDIO_START_READWAIT_DISABLE(__INSTANCE__) \
  (*(__IO uint32_t *)DCTRL_RWSTART_BB = DISABLE)
 ...
 * @retval None
 */
#define __SDIO_STOP_READWAIT_ENABLE(__INSTANCE__) \
  (*(__IO uint32_t *)DCTRL_RWSTOP_BB = ENABLE)
 ...
 * @retval None
 */
#define __SDIO_STOP_READWAIT_DISABLE(__INSTANCE__) \
  (*(__IO uint32_t *)DCTRL_RWSTOP_BB = DISABLE)
 ...
 * @retval None
 */
#define __SDIO_OPERATION_ENABLE(__INSTANCE__) \
 ...
 * @retval None
 */
#define __SDIO_OPERATION_DISABLE(__INSTANCE__) \
  (*(__IO uint32_t *)DCTRL_SDIOEN_BB = DISABLE)
 ...
 * @retval None
 */
#define __SDIO_SUSPEND_CMD_ENABLE(__INSTANCE__) \
  (*(__IO uint32_t *)CMD_SDIOSUSPEND_BB = ENABLE)
 ...
 * @retval None
 */
#define __SDIO_SUSPEND_CMD_DISABLE(__INSTANCE__) \
  (*(__IO uint32_t *)CMD_SDIOSUSPEND_BB = DISABLE)
...
#define __SDIO_CEATA_CMD_COMPLETION_ENABLE() \
  (*(__IO uint32_t *)CMD_ENCMDCOMPL_BB = ENABLE)
...
#define __SDIO_CEATA_CMD_COMPLETION_DISABLE() \
  (*(__IO uint32_t *)CMD_ENCMDCOMPL_BB = DISABLE)
...
#define __SDIO_CEATA_SENDCMD_DISABLE() \
  (*(__IO uint32_t *)CMD_ATACMD_BB = DISABLE)
 ...
 * @{
 */
uint32_t SDMMC_GetCmdResp1(SDIO_TypeDef *SDIOx, uint8_t SD_CMD,
                           uint32_t Timeout);
