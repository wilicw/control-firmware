 ...
 *          - Scope injected group (affects all channels of injected group):
 * InjectedNbrOfConversion, InjectedDiscontinuousConvMode, AutoInjectedConv,
 * ExternalTrigInjecConvEdge, ExternalTrigInjecConv.
 * @note   The setting of these parameters with function
 * HAL_ADCEx_InjectedConfigChannel() is conditioned to ADC state. ADC state can
 * be either:
 *          - For all parameters: ADC disabled
 *          - For all except parameters 'InjectedDiscontinuousConvMode' and
 * 'AutoInjectedConv': ADC enabled without conversion on going on injected
 * group.
 *          - For parameters 'ExternalTrigInjecConv' and
 * 'ExternalTrigInjecConvEdge': ADC enabled, even with conversion on going on
 * injected group.
 */
typedef struct {
  uint32_t
      InjectedChannel;   /*!< Selection of ADC channel to configure
                            ...
                            available on package pins. Refer to device datasheet
                            for channels availability. */
  uint32_t InjectedRank; /*!< Rank in the injected group sequencer
                              This parameter must be a value of @ref
                            ADCEx_injected_rank Note: In case of need to disable
                            a channel or change order of conversion sequencer,
                            rank containing a previous channel setting can be
                            overwritten by the new channel setting (or parameter
  ...
  uint32_t
      InjectedSamplingTime; /*!< Sampling time value to be set for the selected
                               ...
                               time (12 ADC clock cycles at ADC resolution 12
                               bits, 11 cycles at 10 bits, 9 cycles at 8 bits, 7
                               cycles at 6 bits). This parameter can be a value
                               of @ref ADC_sampling_times Caution: This
                               parameter updates the parameter property of the
                               channel, that can be used into regular and/or
                               injected groups. If this same channel has been
                               previously configured in the other group
                               (regular/injected), it will be updated to last
                               setting. Note: In case of usage of internal
                               measurement channels (VrefInt/Vbat/TempSensor),
                                       sampling time constraints must be
                               respected (sampling time can be adjusted in
                               function of ADC clock frequency and sampling time
                               setting) Refer to device datasheet for timings
                               values, parameters TS_vrefint, TS_temp (values
                               rough order: 4us min). */
  uint32_t InjectedOffset; /*!< Defines the offset to be subtracted from the raw
                              converted data (for channels set on injected group
                              only). Offset value must be a positive number.
                                Depending of ADC resolution selected (12, 10, 8
                              or 6 bits), this parameter must be a number
  ...
  uint32_t
      InjectedNbrOfConversion; /*!< Specifies the number of ranks that will be
                                  converted within the injected group sequencer.
                                    To use the injected group sequencer and
                                  convert several ranks, parameter
  ...
  FunctionalState
      InjectedDiscontinuousConvMode; /*!< Specifies whether the conversions
                                        sequence of injected group is performed
                                        in
                                        Complete-sequence/Discontinuous-sequence
                                        (main sequence subdivided in successive
                                        parts). Discontinuous mode is used only
                                        if sequencer is enabled (parameter
                                        ...
                                        continuous mode is enabled, this
                                        parameter setting is discarded. This
                                        parameter can be set to ENABLE or
                                        DISABLE. Note: For injected group,
                                        number of discontinuous ranks increment
                                        is fixed to one-by-one. Caution: this
                                        ...
                                        previously set. */
  FunctionalState
      AutoInjectedConv; /*!< Enables or disables the selected ADC automatic
                           injected group conversion after regular one This
                           parameter can be set to ENABLE or DISABLE. Note: To
                           use Automatic injected conversion, discontinuous mode
                           ...
                           ('ExternalTrigInjecConv' set to ADC_SOFTWARE_START)
  ...
  uint32_t
      ExternalTrigInjecConv; /*!< Selects the external event used to trigger the
                                ...
                                are disabled. If set to external trigger source,
                                triggering is on event rising edge. This
                                parameter can be a value of @ref
                                ADCEx_External_trigger_Source_Injected Note:
                                This parameter must be modified when ADC is
                                disabled (before ADC start conversion or after
                                ADC stop conversion). If ADC is enabled, this
                                parameter setting is bypassed without error
                                reporting (as it can be the expected behaviour
                                in case of another parameter update on the fly)
  ...
  uint32_t
      ExternalTrigInjecConvEdge; /*!< Selects the external trigger edge of
                                    ...
                                    value of @ref
                                    ADCEx_External_trigger_edge_Injected. If
                                    trigger is set to
...
typedef struct {
  ...
  uint32_t
      DMAAccessMode; /*!< Configures the Direct memory access mode for multi ADC
...
/** @defgroup ADCEx_Direct_memory_access_mode_for_multi_mode ADC Direct Memory
 * Access Mode For Multi Mode
 * @{
 */
#define ADC_DMAACCESSMODE_DISABLED 0x00000000U /*!< DMA mode disabled */
#define ADC_DMAACCESSMODE_1                                                  \
  ((uint32_t)ADC_CCR_DMA_0) /*!< DMA mode 1 enabled (2 / 3 half-words one by \
                               one - 1 then 2 then 3)*/
#define ADC_DMAACCESSMODE_2                                                    \
  ((uint32_t)ADC_CCR_DMA_1) /*!< DMA mode 2 enabled (2 / 3 half-words by pairs \
                               - 2&1 then 1&3 then 3&2)*/
#define ADC_DMAACCESSMODE_3                                                   \
  ((uint32_t)ADC_CCR_DMA) /*!< DMA mode 3 enabled (2 / 3 bytes by pairs - 2&1 \
                             then 1&3 then 3&2) */
/**
 * @}
 */

/** @defgroup ADCEx_External_trigger_edge_Injected ADC External Trigger Edge
 * Injected
 * @{
 */
...
/** @defgroup ADCEx_External_trigger_Source_Injected ADC External Trigger Source
 * Injected
 * @{
 */
...
#define ADC_CHANNEL_TEMPSENSOR \
  ((uint32_t)ADC_CHANNEL_18 | ADC_CHANNEL_DIFFERENCIATION_TEMPSENSOR_VBAT)
#endif /* STM32F411xE || STM32F413xx || STM32F423xx || STM32F427xx || \
...
#define ADC_JSQR(_CHANNELNB_, _RANKNB_, _JSQR_JL_) \
  (((uint32_t)((uint16_t)(_CHANNELNB_)))           \
   << (5U * (uint8_t)(((_RANKNB_) + 3U) - (_JSQR_JL_))))

/**
 * @brief Defines if the selected ADC is within ADC common register ADC123 or
 * ADC1 if available (ADC2, ADC3 availability depends on STM32 product)
