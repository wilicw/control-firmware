 ...
 *          - Scope entire ADC (affects regular and injected groups):
 * ClockPrescaler, Resolution, ScanConvMode, DataAlign, ScanConvMode,
 ...
 * ExternalTrigConv.
 * @note   The setting of these parameters with function HAL_ADC_Init() is
 * conditioned to ADC state. ADC state can be either:
 *          - For all parameters: ADC disabled
 *          - For all parameters except 'Resolution', 'ScanConvMode',
 * 'DiscontinuousConvMode', 'NbrOfDiscConversion' : ADC enabled without
 * conversion on going on regular group.
 *          - For parameters 'ExternalTrigConv' and 'ExternalTrigConvEdge': ADC
 * enabled, even with conversion on going. If ADC is not in the appropriate
 * state to modify some parameters, these parameters setting is bypassed without
 * error reporting (as it can be the expected behaviour in case of intended
 * action to update another parameter (which fulfills the ADC state condition)
 * on the fly).
 */
typedef struct {
  uint32_t ClockPrescaler; /*!< Select ADC clock prescaler. The clock is common
                              for all the ADCs. This parameter can be a value of
                              @ref ADC_ClockPrescaler */
  uint32_t
      Resolution; /*!< Configures the ADC resolution.
                       This parameter can be a value of @ref ADC_Resolution */
  uint32_t
      DataAlign; /*!< Specifies ADC data alignment to right (MSB on register bit
                    11 and LSB on register bit 0) (default setting) or to left
                    (if regular group: MSB on register bit 15 and LSB on
                    register bit 4, if injected group (MSB kept as signed value
                    due to potential negative value after offset application):
                    MSB on register bit 14 and LSB on register bit 3). This
                    parameter can be a value of @ref ADC_Data_align */
  uint32_t
      ScanConvMode; /*!< Configures the sequencer of regular and injected
                       groups. This parameter can be associated to parameter
                       ...
                       'NbrOfConversion'/'InjectedNbrOfConversion' and each
                       channel rank). Scan direction is upward: from rank1 to
                       rank 'n'. This parameter can be set to ENABLE or DISABLE
                     */
  uint32_t
      EOCSelection; /*!< Specifies what EOC (End Of Conversion) flag is used for
                       conversion by polling and interruption: end of conversion
                       of each rank or complete sequence. This parameter can be
                       a value of @ref ADC_EOCSelection. Note: For injected
                       group, end of conversion (flag&IT) is raised only at the
                       end of the sequence. Therefore, if end of conversion is
                       set to end of each conversion, injected group should not
                       be used with interruption (HAL_ADCEx_InjectedStart_IT) or
                       polling (HAL_ADCEx_InjectedStart and
                       HAL_ADCEx_InjectedPollForConversion). By the way, polling
                       is still possible since driver will use an estimated
                       timing for end of injected conversion. Note: If overrun
                       feature is intended to be used, use ADC in mode
                       'interruption' (function HAL_ADC_Start_IT() ) with
                       parameter EOCSelection set to end of each conversion or
                       in mode 'transfer by DMA' (function HAL_ADC_Start_DMA()).
                               If overrun feature is intended to be bypassed,
                       use ADC in mode 'polling' or 'interruption' with
  ...
  FunctionalState
      ContinuousConvMode; /*!< Specifies whether the conversion is performed in
                             ...
                             regular group, after the selected trigger occurred
                             (software start or external trigger). This
                             parameter can be set to ENABLE or DISABLE. */
  uint32_t
      NbrOfConversion; /*!< Specifies the number of ranks that will be converted
                          within the regular group sequencer. To use regular
  ...
  uint32_t
      NbrOfDiscConversion; /*!< Specifies the number of discontinuous
                              conversions in which the  main sequence of regular
                              group (parameter NbrOfConversion) will be
  ...
  uint32_t
      ExternalTrigConv; /*!< Selects the external event used to trigger the
                           ...
                           ADC_SOFTWARE_START, external triggers are disabled.
                             If set to external trigger source, triggering is on
                           event rising edge by default. This parameter can be a
                           value of @ref ADC_External_trigger_Source_Regular */
  uint32_t ExternalTrigConvEdge; /*!< Selects the external trigger edge of
                                    regular group. If trigger is set to
                                    ADC_SOFTWARE_START, this parameter is
                                    discarded. This parameter can be a value of
                                    @ref ADC_External_trigger_edge_Regular */
  FunctionalState
      DMAContinuousRequests; /*!< Specifies whether the DMA requests are
                                performed in one shot mode (DMA transfer stop
                                when number of conversions is reached) or in
                                Continuous mode (DMA transfer unlimited,
                                whatever number of conversions). Note: In
                                continuous mode, DMA must be configured in
                                circular mode. Otherwise an overrun will be
                                triggered when DMA buffer maximum pointer is
                                reached. Note: This parameter must be modified
                                when no conversion is on going on both regular
                                and injected groups (ADC disabled, or ADC
                                enabled without continuous mode or external
                                trigger that could launch a conversion). This
                                parameter can be set to ENABLE or DISABLE. */
} ADC_InitTypeDef;

/**
 * @brief  Structure definition of ADC channel for regular group
 * @note   The setting of these parameters with function HAL_ADC_ConfigChannel()
 * is conditioned to ADC state. ADC can be either disabled or enabled without
 * conversion on going on regular group.
 */
typedef struct {
  uint32_t
  ...
  uint32_t
      SamplingTime; /*!< Sampling time value to be set for the selected channel.
                       ...
                       processing time (12 ADC clock cycles at ADC resolution 12
                       bits, 11 cycles at 10 bits, 9 cycles at 8 bits, 7 cycles
                       at 6 bits). This parameter can be a value of @ref
                       ADC_sampling_times Caution: This parameter updates the
                       parameter property of the channel, that can be used into
                       regular and/or injected groups. If this same channel has
                       been previously configured in the other group
                       (regular/injected), it will be updated to last setting.
                         Note: In case of usage of internal measurement channels
                       (VrefInt/Vbat/TempSensor), sampling time constraints must
                       be respected (sampling time can be adjusted in function
                       of ADC clock frequency and sampling time setting) Refer
                       to device datasheet for timings values, parameters
...
#define HAL_ADC_STATE_RESET \
  0x00000000U /*!< ADC not yet initialized or disabled */
#define HAL_ADC_STATE_READY 0x00000001U /*!< ADC peripheral ready for use */
#define HAL_ADC_STATE_BUSY_INTERNAL                                 \
  0x00000002U /*!< ADC is busy to internal process (initialization, \
                 calibration) */
#define HAL_ADC_STATE_TIMEOUT 0x00000004U /*!< TimeOut occurrence */

/* States of ADC errors */
#define HAL_ADC_STATE_ERROR_INTERNAL \
  0x00000010U /*!< Internal error occurrence */
#define HAL_ADC_STATE_ERROR_CONFIG \
  0x00000020U /*!< Configuration error occurrence */
...
#define HAL_ADC_STATE_AWD2                                                   \
  0x00020000U /*!< Not available on STM32F4 device: Out-of-window occurrence \
                 of analog watchdog 2 */
#define HAL_ADC_STATE_AWD3                                                   \
  0x00040000U /*!< Not available on STM32F4 device: Out-of-window occurrence \
                 of analog watchdog 3 */

/* States of ADC multi-mode */
#define HAL_ADC_STATE_MULTIMODE_SLAVE                                      \
  0x00100000U /*!< Not available on STM32F4 device: ADC in multimode slave \
                 state, controlled by another ADC master ( */
...
/** @defgroup ADC_delay_between_2_sampling_phases ADC Delay Between 2 Sampling
 * Phases
 * @{
 */
...
/** @defgroup ADC_External_trigger_edge_Regular ADC External Trigger Edge
 * Regular
 * @{
 */
...
/** @defgroup ADC_External_trigger_Source_Regular ADC External Trigger Source
 * Regular
 * @{
 */
...
#define ADC_CHANNEL_11 \
  ((uint32_t)(ADC_CR1_AWDCH_3 | ADC_CR1_AWDCH_1 | ADC_CR1_AWDCH_0))
#define ADC_CHANNEL_12 ((uint32_t)(ADC_CR1_AWDCH_3 | ADC_CR1_AWDCH_2))
#define ADC_CHANNEL_13 \
  ((uint32_t)(ADC_CR1_AWDCH_3 | ADC_CR1_AWDCH_2 | ADC_CR1_AWDCH_0))
...
#define ADC_SAMPLETIME_56CYCLES \
  ((uint32_t)(ADC_SMPR1_SMP10_1 | ADC_SMPR1_SMP10_0))
#define ADC_SAMPLETIME_84CYCLES ((uint32_t)ADC_SMPR1_SMP10_2)
#define ADC_SAMPLETIME_112CYCLES \
  ((uint32_t)(ADC_SMPR1_SMP10_2 | ADC_SMPR1_SMP10_0))
#define ADC_SAMPLETIME_144CYCLES \
  ((uint32_t)(ADC_SMPR1_SMP10_2 | ADC_SMPR1_SMP10_1))
...
#define ADC_ANALOGWATCHDOG_SINGLE_REG \
  ((uint32_t)(ADC_CR1_AWDSGL | ADC_CR1_AWDEN))
#define ADC_ANALOGWATCHDOG_SINGLE_INJEC \
  ((uint32_t)(ADC_CR1_AWDSGL | ADC_CR1_JAWDEN))
...
#define ADC_IS_ENABLE(__HANDLE__)                                        \
  (((((__HANDLE__)->Instance->SR & ADC_SR_ADONS) == ADC_SR_ADONS)) ? SET \
                                                                   : RESET)
...
/**
 * @brief  Configures the number of discontinuous conversions for the regular
 * group channels.
