...
typedef struct {
  ...
  uint32_t FIFOMode; /*!< Specifies if the FIFO mode or Direct mode will be used
                        for the specified stream. This parameter can be a value
                        of @ref DMA_LL_FIFOMODE
                          @note The Direct mode (FIFO mode disabled) cannot be
                        used if the memory-to-memory data transfer is configured
                        on the selected stream

                          This feature can be modified afterwards using unitary
                        functions @ref LL_DMA_EnableFifoMode() or @ref
  ...
  uint32_t MemBurst; /*!< Specifies the Burst transfer configuration for the
                        memory transfers. It specifies the amount of data to be
...
#define LL_DMA_OFFSETSIZE_FIXEDTO4                                            \
  DMA_SxCR_PINCOS /*!< Peripheral increment offset size is fixed to 4 (32-bit \
                     alignment) */
/**
 * @}
 */
...
#define LL_DMA_MBURST_INC16 \
  (DMA_SxCR_MBURST_0 |      \
   DMA_SxCR_MBURST_1) /*!< Memory burst of 16 beats transfer configuration */
/**
 * @}
 */
...
__STATIC_INLINE uint32_t LL_DMA_GetMemoryAddress(DMA_TypeDef *DMAx,
                                                 uint32_t Stream) {
  return (READ_REG(((DMA_Stream_TypeDef *)((
                        uint32_t)((uint32_t)DMAx + STREAM_OFFSET_TAB[Stream])))
                       ->M0AR));
}
