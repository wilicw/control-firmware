...
RETURN_CODE_ERROR=1
FILE_LIST="$(find "$SCRIPT_DIRECTORY/Core" "$SCRIPT_DIRECTORY/Drivers" "$SCRIPT_DIRECTORY/FileX" "$SCRIPT_DIRECTORY/AZURE_RTOS" "$SCRIPT_DIRECTORY/FSAE" | grep -E ".*\.(ino|cpp|cc|c|h|hpp|hh)$")"
